...
import ESG from '@/projects/esg-dashboard.webp'
...
import LICENTRA from '@/projects/licentra.webp'
...
export const MAJOR_PROJECTS: TProject[] = [
  {
    ...
    color: '#059669',
    summary:
      'ESG Dashboard is an enterprise ESG maturity platform built for Indonesian corporate groups (IDSurvey / PT Surveyor Indonesia). Organizations across a hierarchical company tree complete ESG assessments per reporting period; the platform tracks maturity levels reached, rolls up GHG emissions (Scope 1/2/3), and delivers executive dashboards, cross-unit comparisons, geographic emission heatmaps, and ESG ratings. Built as a Turborepo monorepo with React 19 + NestJS 11 + tRPC + Prisma, it enforces role- and org-scoped access (Admin, Coordinator, Staff, Viewer), secure httpOnly session authentication, evidence document management, audit logging, and production-ready Docker/Traefik deployment with CI/CD. The product focuses on ESG maturity level achieved—not mere checklist completion percentage.',
    links: [],
    features: [
      'Hierarchical organization tree (HOLDING → COMPANY → SUBSIDIARY → HEAD_OFFICE → BRANCH → LAB) with subtree-scoped access',
      'Role-based access control: Admin, Coordinator, Staff, and Viewer',
      'Reporting periods (ANNUAL / QUARTERLY) with full lifecycle: DRAFT → OPEN → CLOSED → REOPENED → ARCHIVED',
      'ESG maturity assessment via master checklist templates and company-scoped template versions',
      'Checklist builder with versioning (DRAFT / PUBLISHED) and revision history',
      'Staff work assignments with draft autosave, level unlocking, and evidence attachments',
      'Coordinator monitoring & review of submissions across the org subtree',
      'GHG emission management (Scope 1/2/3) with manual entry and bulk Excel import',
      'Automatic normalization of emissions to energy (GJ) and CO₂ (kg) via emission factors',
      'ENV-01-02 emission rollup seam into executive dashboards',
      'Executive platform dashboard: maturity snapshots, progress, YoY trends, and org comparisons',
      'Geographic emission choropleth / heatmap (MapLibre)',
      'ESG rating & industry benchmark scorecards (manually entered ratings)',
      'Evidence & document library with S3-compatible object storage (MinIO)',
      'Audit log with before/after diffs for accountability',
      'Deadline notifications and optional quarterly emission reminders (SMTP)',
      'Configurable platform branding / workspace settings (name, SEO preview)',
      'Secure session auth (HMAC-SHA256, httpOnly cookie, DB-backed UserSession)',
      'Type-safe end-to-end API via tRPC + Zod validation',
      'Bulk user & organization import/export via Excel',
      'Roadmap / recommendation tracking for ESG improvement plans',
      'Health, readiness, and metrics endpoints for production ops',
      'Docker + Traefik deployment with GitHub Actions CI/CD and automated VPS deploy',
      'Performance (k6) and security (OWASP ZAP baseline, dependency audit) test tooling',
    ],
    techs: [
      'react',
      'typescript',
  ...
  },
  {
    ...
    links: [],
    features: [
      'Windows Go agent: registry apps, running processes, and filesystem executables',
      'Device registration via unit-scoped registration tokens (no shared global credentials)',
      'Mass deployment bundle: ZIP with Licentra.exe / Licentra-x86.exe, config.json, install.ps1, bilingual README',
      'USB / flash-disk friendly install flow for large workstation fleets',
      'Per-device API key auth with rotation and grace period',
      'Agent heartbeat monitoring and fleet health dashboard',
      'In-place agent auto-update (amd64 & 386) with SHA-256 manifest verification',
      'Large audit submission ingest with queue-based processing (BullMQ / Redis)',
      'Optional HMAC-SHA256 signing on audit submit and action payloads',
      'Compliance scoring, snapshots, and configurable compliance policy (weights & thresholds)',
      'Software catalog governance with categories, versions, risk levels, and aliases',
      'Catalog candidate aggregation with human-in-the-loop approve/reject',
      'Unit software baselines (mandatory / prohibited) with candidate review queues',
      'Licence pools and seat assignments tied to catalog titles and devices',
      'Suspicious / unknown software triage queue with single and batch actions',
      'Rules engine, vendor whitelist, rule simulation, and fleet reclassification',
      'Self-improving intelligence: baselines, risk proposals, aliases, rule optimisations, co-occurrence graph',
      'AI-assisted rule candidates and nightly classification pipelines (LLM-backed where configured)',
      'Natural-language AI query over device / anomaly scope',
      'Offline malware hash matching (abuse.ch feed) against inventory digests',
      'Action queue: investigate / rescan / sync with timeline and status transitions',
      'Cross-fleet asset inventory (“where is this software installed?”)',
      'Hierarchical organisational units: create, split, merge, assign devices',
      'SI Portal sync for units and users (when configured)',
      'RBAC with roles (superadmin, it_admin, it_staff, unit_manager, viewer) and unit tree scope',
      'JWT access/refresh session auth for the dashboard',
      'Compliance reports: PDF / CSV / XLSX via queued workers, stored on MinIO (S3)',
      'Executive compliance PDF generation (React PDF pipeline)',
      'Activity audit log merging submissions and action history',
      'Configurable data retention policies with cleanup runs',
      'In-app notifications and user preference settings',
      'Prometheus metrics endpoint for operations',
      'Docker Compose + Traefik production deploy; agent cross-compile, Authenticode/osslsigncode signing, and publish scripts',
    ],
    techs: [
      'typescript',
      'react',
  ...
  },
  {
    ...
    color: '#3B82F6',
    summary:
      'A comprehensive Flutter-based helpdesk application for PT Surveyor Indonesia (PTSI) featuring a Performance-Based Ticket Management System with advanced SLA tracking, priority matrix, and dual feedback system. The app improves IT support ticket management with auto-calculated priorities, real-time SLA countdown timers, and comprehensive performance analytics. Built with clean architecture and BLoC pattern, it ensures scalable, maintainable code while delivering a solid user experience across Android and iOS platforms.',
    links: [],
    features: [
